 ...
 * @date 2019/3/19
 */
public class AuthSampleActivity extends AppCompatActivity {
    ...
    private AuthActionCallBack authActionCallBack = new AuthActionCallBack() {
        ...
        public void onComplete(int platform, String authJson) {
            ...
            if (platform == ShareManager.PLATFORM_QQ)
                ResultActivity.startActivity(AuthSampleActivity.this, "QQ授权登录成功，授权结果是：" + authJson);
        }

        @Override
        public void onCancel(int platform) {
            if (platform == ShareManager.PLATFORM_QQ)
                ResultActivity.startActivity(AuthSampleActivity.this, "QQ授权登录取消");
        }

        @Override
        public void onError(int platform, Throwable var3) {
            if (platform == ShareManager.PLATFORM_QQ)
                ResultActivity.startActivity(AuthSampleActivity.this, "QQ授权登录失败，失败原因：" + var3.getMessage());
        }
    };
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        findViewById(R.id.wx).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                Toast.makeText(AuthSampleActivity.this, "南通百通没有开通微信快捷登录", Toast.LENGTH_LONG).show();
            }
        });
        findViewById(R.id.qq).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                ShareManager.getInstance().authorizeForQQ(AuthSampleActivity.this, authActionCallBack);
            }
        });
